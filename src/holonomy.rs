...
impl HolonomyGroup {
    ...
    pub fn rectangular_cycle(
        omega: &ConnectionForm,
        width: f64,
        height: f64,
        steps: usize,
    ) -> Self {
...
mod tests {
    ...
    fn holonomy_drift_equals_drift_times_cycle_length() {
        ...
        let h = HolonomyGroup::around_cycle(
            &[omega.clone(), omega.clone(), omega.clone(), omega.clone()],
            cycle_length,
            100,
        );
        ...
        let omega2 = ConnectionForm::new(vec![vec![0.0, delta], vec![-delta, 0.0]]);
